...
void Timer_Init(timer_init_cfg_t* timer_init_cfg)
{
    if(timer_init_cfg != (0))
    {
        ...
        if(timer_init_cfg -> channel == (1))
        {
             if(timer_init_cfg -> mode == (1))
                {
                    OPTION_REG &=~(1 << 5);
                    OPTION_REG &=~(1 << 3);
                     TMR0 = 100;
                    ...
                    switch(timer_init_cfg ->prescaler)
                        ...
                        if(timer_init_cfg ->interrupt_mode == (1))
                           {
                               INTCON &=~((1 << 7)|(1 << 5));
                           }
                        else if(timer_init_cfg ->interrupt_mode == (0))
                           {
                               INTCON |=((1 << 7)|(1 << 5));
                               if(timer_init_cfg -> edge == (1))
                                    {
                                        OPTION_REG &= ~(1 << 4);
                                    }
                                else if(timer_init_cfg -> edge == (0))
                                    {
                                           OPTION_REG |= (1 << 4);
                                    }
                                else
                        ...
                        else if(timer_init_cfg -> mode == (0))
                            {

                                OPTION_REG |=(1 << 5);
                                dio_write(GPIO_A,0x10,(0));
                            }
                        else
                            {
